...
def costars(name)
  ...
  # appeared with.
  # Hint: use a subquery
  movie_ids = Movie.select(:id).joins(:actors).where(actors: {name: name})

  Movie.joins(:actors).where(movies: {id: movie_ids}).where.not(actors: {name: name}).distinct.pluck(:name)
